...
On MacOS, download StreamDeck2027-Mac.tar.gz from the [latest release](https://github.com/karlolson5/StreamDeck2027/releases), then extract the file, then run `xattr -d com.apple.quarantine StreamDeck2027-Mac` in the folder containing the app. This is a necsesary step because I refuse to pay $99 for a developer license.
...
### Network Target
By default, the network target is 10.34.76.2, the robot controller IP address for FRC Team 3476 Code Orange. If you check the "Target simulated robot" checkbox, the network target will change to 127.0.0.1, the default local network for simulated FRC robots using WPILib simulation.
